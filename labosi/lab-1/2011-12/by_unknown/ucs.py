...
def expand(stanje):
    if stanje[2]=='L':
        ...
        for i in range (len(all_permut)):
            ...
            if visited.has_key(str(new_state)):
                pass
            else:
                visited[str(new_state)] = new_state_price
                new_state.append(new_state_price)
                open_list.put((new_state[3],new_state))
...
def main():
    ...
    #lijevi = [19,1,2,15,5,15,9]
    #lijevi = [1,2,5,10,12,17,24,21,22,13]
    #lijevi = [7,7,7,1,4,13]
    #lijevi = [1,2,5,10]
    desni = list()
    flag = 'L'
    ...
    while not open_list.empty():
        ...
        expand(current[1])   
    print "minimalna cijena: ",minimum
